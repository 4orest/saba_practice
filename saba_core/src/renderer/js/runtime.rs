    ...
    fn eval(
        ...
        match node.borrow() {
            ...
            Node::MemberExpression { object, property } => {
                ...
                // という一つの文字列として扱う。
                // このメソッドへの呼び出しは、"document.getElementById"
                // という名前の関数への呼び出しになる
                return Some(
            ...
            Node::CallExpression { callee, arguments } => {
                // 新しいスコープを作成する
                let new_nev = Rc::new(RefCell::new(Environment::new(Some(env))));

                let callee_value = match self.eval(callee, new_nev.clone()) {
                    Some(value) => value,
                    None => return None,
                };

                // ブラウザAPIの呼び出しを試みる
                let api_result = self.call_browser_api(&callee_value, arguments, new_nev.clone());
                if api_result.0 {
                    // もしブラウザAPIを呼び出していたら、ユーザーが定義した関数は
                    // 実行しない
                    return api_result.1;
                }
                ...
                for (i, item) in arguments.iter().enumerate() {
                    if let Some(RuntimeValue::StringLiteral(name)) =
                        self.eval(&function.params[i], new_nev.clone())
                    {
                        new_nev
                            .borrow_mut()
                            .add_variable(name, self.eval(item, new_nev.clone()))
                    }
                }

                // 関数の中身を新しいスコープとともにevalメソッドで解釈する
                self.eval(&function.body.clone(), new_nev.clone())
            }
        }
    }

    /// (bool, Option<RuntimeValue>)のタプルを返す
    /// bool: ブラウザAPIが呼ばれたかどうか。trueなら何かしらのAPIが呼ばれた
    /// ことを示す
